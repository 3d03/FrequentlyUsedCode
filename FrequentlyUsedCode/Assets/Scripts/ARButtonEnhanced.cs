...
public class ARButtonEnhanced : MonoBehaviour
{
    [Space(20)]
    ...
    [Space(20)]
    public bool longClickEnabled;
    private int longClickCounter;
    [SerializeField]
    private float longClickHoldTime=1f;
        ...
        if (isEnabled)
            if (!EventSystem.current.IsPointerOverGameObject())
                if (longClickEnabled)
                {
                    if (pointerDown)
                    {
                        pointerDownTimer = pointerDownTimer + Time.deltaTime;

                        if (pointerDownTimer > longClickHoldTime)
                        {
    ...
    void LongClick()
    {
        if (!EventSystem.current.IsPointerOverGameObject())
        {
            if (longClickEvents[longClickCounter] != null)
            {
                longClickEvents[longClickCounter].Invoke();
            }
            longClickCounter++;
            if (longClickCounter == longClickEvents.Length)
            {
                longClickCounter = 0;
            }
        }
    ...
    public void InvokeLongClickEvents(int index)
    {
        if (index < longClickEvents.Length)
            longClickEvents[index].Invoke();
    }
    #endregion
}
